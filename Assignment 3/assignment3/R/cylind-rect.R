#' @title Cylindrical to Rectangular Coordinate Converter
#' @description
#' Converts cylindrical (r,theta,z) coordinates to (x,y,z) rectangular coordinates
#' and returns coordinates as a vector
#' @param r r value for the cylindrical coordinate of interest
#' @param theta theta value for the cylindrical coordinate of interest
#' @param z z value for the cylindircal coordinate of interest
#' @export
#' @return rectangular
#' @examples \dontrun{
#' cylindricaltorecangular(r,theta,z)
#' }

cylindricaltorectangular <- function (r,theta,z) {
  # Error Handling (Lines 15-79)
  # Determines if any arguments sent by the user are non-numeric
  nonnumeric <- c(typeof(r),typeof(theta),typeof(z))
  # Determines if any arguments sent by the user are blank
  missing_vars <- c(missing(r),missing(theta),missing(z))
  ...
  # Calls the polar to rectangular function to convert the (r,theta) polar coordinates to
  # (x,y) rectangular coordinates
  xy <- polartorectangular(r,theta)
  # Assigns converted (x,y) coordinates with the associated z coordinate in the vector
  # 'rectangular' for reporting to user
  rectangular <- c(xy[1],xy[2],z)
  # Returns converted coordinates to user
  return(rectangular)
}
