#' @title Cylindrical to Spherical Coordinate Converter
#' @description
#' Converts cylindrical (r,theta,z) coordinates to (rho,theta,phi) spherical coordinates
...
#' @param z z value for the cylindrical coordinate of interest
#' @export
#' @return spherical
#' @examples \dontrun{
...
cylindricaltospherical <- function(r,theta,z){
  ...
  # Calls the magnitude function to calculate rho based on rectangular (x,y,z) coordinates
  rho <- magnitude(r,z)
  # Calls the phicalc function to calculate angle phi (in radians) based on rectangular coordinates
  phi <- phicalc(z,rho)
  # Assigns calculated rho and phi values, alongside provided theta value, to vector spherical
  spherical <- c(rho,theta,phi)
  # Returns spherical
  return(spherical)
}
